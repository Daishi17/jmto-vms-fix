...
<body style="font-size: 18px; text-align:justify">
    <div class="container">
        ...
        <center>
            <h4>SURAT PERNYATAAN MINAT </h4>
        </center>
        <br>
        <label for="">Kami atas nama Pengurus <b><?= $nama_usaha ?></b> dalam rangka <b><?= $rup['nama_rup'] ?></b> menyatakan dengan sesungguhnya bahwa setelah mengikuti Rapat Penjelasan Pra Kualifikasi <b><?= $rup['nama_rup'] ?></b> , dengan ini <b>Menyatakan Minat</b> untuk mengikuti setiap tahapan Pengadaan Barang/Jasa dimaksud sampai dengan selesai diumumkannya Pemenang.</label>
        ...
        <div class="float-right mt-5">
            ...
            <?php } else { ?>
                ...
                <?php    } else if (date('Y-m-d H:i', strtotime($jadwal_upload_dokumen_prakualifikasi['waktu_selesai'])) >= date('Y-m-d H:i') || date('Y-m-d H:i', strtotime($jadwal_upload_dokumen_prakualifikasi['waktu_mulai'])) == date('Y-m-d H:i')) { ?>
                    ...
                    <?php } else { ?>
                        <center>
                            <a href="javascript:;" onclick="setujui_surat('<?= $rup['id_rup'] ?>','sts_suratpernyataan_2')" class="btn btn-sm btn-danger">Silahkan Disetujui (Klik Tombol Ini)</a>
                        </center>
                    <?php } ?>
                <?php  } else { ?>
