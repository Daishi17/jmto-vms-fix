...
<head>
    <title>SURAT PERNYATAAN KEBENARAN DATA DALAM FORMULIR ISIAN KUALIFIKASI
    </title>
    <meta charset="utf-8">
        ...
        <div class="float-right mt-5">
            ...
            <?php } else { ?>
                ...
                <?php    } else if (date('Y-m-d H:i', strtotime($jadwal_upload_dokumen_prakualifikasi['waktu_selesai'])) >= date('Y-m-d H:i') || date('Y-m-d H:i', strtotime($jadwal_upload_dokumen_prakualifikasi['waktu_mulai'])) == date('Y-m-d H:i')) { ?>
                    ...
                    <?php } else { ?>
                        <center>
                            <a href="javascript:;" onclick="setujui_surat('<?= $rup['id_rup'] ?>','sts_suratpernyataan_3')" class="btn btn-sm btn-danger">Silahkan Disetujui (Klik Tombol Ini)</a>
                        </center>
                    <?php } ?>
                <?php  } else { ?>
